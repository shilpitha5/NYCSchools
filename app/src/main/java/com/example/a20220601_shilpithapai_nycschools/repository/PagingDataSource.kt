...
class PagingDataSource(private val apiService: ApiService) : PagingSource<Int, School>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, School> {
        ...
        val offset = if (params.key == null) (position - 1) * TOTAL_ITEM_TO_BE_DISPLAYED else 0
        ...
        return try {
            ...
            LoadResult.Page(
                data = response,
                prevKey = null,
                nextKey = if (response.isEmpty()) null else (params.loadSize / TOTAL_ITEM_TO_BE_DISPLAYED) + position
            )
